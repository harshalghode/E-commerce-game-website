...
<?php 
   $st = $conn-> prepare("SELECT * FROM user WHERE uid=?");
   $st->bind_param("i",$uid);
...
<div class="container">
  <div class="row justify-content-center">
          ...
          <h6 class="text-center">Billing Details :-</h6>
  				<div class="form-group">

  					<input type="text" name="name" class="form-control" value="<?= $name; ?>" placeholder="Enter Name" required>
  				</div>
  				<div class="form-group">
  					<input type="email" name="email" class="form-control" value="<?= $ro['email']; ?>" placeholder="Enter Email" required>
  				</div>
  				
  				<div class="form-group">
  					<textarea name="address" class="form-control" rows="3" cols="10" placeholder="<?= $ro['address']; ?>"></textarea>
  				</div>
